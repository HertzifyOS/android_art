...
class VirtualThreadMounter {
 public:
  explicit VirtualThreadMounter(MountedVirtualThreadData* mounted_data) REQUIRES_SHARED(Locks::mutator_lock_) {
    Thread* self = Thread::Current();
...
TEST_F(MonitorTest, TestMonitorOwner) {
  ...
  EXPECT_TRUE(owner.IsVirtual());
  EXPECT_EQ(virtual_thread_id, owner.GetVirtualId());
